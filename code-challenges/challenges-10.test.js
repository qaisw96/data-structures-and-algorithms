...
const count = (target, input) => {
  // Solution code here...
  let times = 0;
  input.filter((element, ind) => {
    element.filter((elm, inx) => {
      if(elm === target) {
        times++
      }
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.map(element=>{
    newArr.push(element.filter(elm => { 
      return (elm*1) && !(elm % 5)} ).map(el => {
...
let findShortest = (data) => {
  // Solution code here...
  let num = 0
  data.filter((element, ind) => {
    ind++
    if(element.height < data[ind].height) {
      return element.name
    } 
};
